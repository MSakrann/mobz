...
import { useScroll } from "@house/hooks/smooth-scroll/use-scroll";
import {
  PRELOADER_COUNT,
  PRELOADER_FADE,
  PRELOADER_RULE,
  PRELOADER_WIPE,
} from "@house/lib/springs/presets";
...
export const Preloader = ({ label }: PreloaderProps) => {
  ...
  return (
    <div aria-hidden className="fixed inset-0 z-200 text-foreground">
      <div className="absolute inset-0 flex">
        {COLUMNS.map((column) => (
          <Spring
            ...
            className={`h-full flex-1 bg-background ${column % 2 === 0 ? "origin-top" : "origin-bottom"}`}
          />
        ))}
      </div>
      ...
      <Spring
        ...
        className="absolute left-10 top-10 text-title-phone uppercase tracking-title sm:text-title max-md:left-6 max-md:top-6"
      >
        {label}
      </Spring>
      ...
      <Spring
        ...
        className="absolute inset-x-10 top-1/2 h-px -translate-y-1/2 origin-right bg-foreground/15 max-md:inset-x-6"
      >
        <animated.span className="block h-full origin-left bg-foreground" style={{ transform: rule }} />
      </Spring>
