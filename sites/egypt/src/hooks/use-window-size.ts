/**
 * @fileoverview Hook for accessing and responding to the window width.
 *
 * It reads from a **shared store**: one debounced `resize` listener is attached
 * for the whole app (on the first mounted consumer) and removed when the last
 * consumer unmounts — instead of one listener + one debounce timer per hook
 * call. State is delivered through `useSyncExternalStore`, so it is SSR-safe
 * and tear-free. The store still measures both axes, which is what keeps a
 * height reader a two-line addition if one is ever needed.
 */
