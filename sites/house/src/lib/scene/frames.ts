 ...
 *
 * The source shipped 210 × 2560×1440 PNGs (~6 MB each, 1.3 GB in all). They are
 * re-encoded offline to WebP in two tiers; see the changelog entry for the
 * commands. The binding constraint on a phone is memory, not bandwidth — a
 * decoded 1280×720 frame is 3.7 MB whatever it cost on the wire — so the
 * mobile tier halves the count as well as the size.
 */
...
export const FRAME_TIERS = {
  desktop: { name: "desktop", count: 210, width: 1920, height: 1080 },
  mobile: { name: "mobile", count: 105, width: 1280, height: 720 },
...
export const framePath = (tier: FrameTier, index: number): string =>
  `/assets/house/scene/sequence/${tier.name}/${String(index).padStart(3, "0")}.webp`;
